    ...
    if($ispost) {
        ...
        if(empty($error)) {
            ...
            $config = str_replace(array(
               '{cfg-server}','{cfg-client}',  '{db-server}', '{db-username}', '{db-password}', '{db-port}', '{db-name}', '{db-tablepre}', '{cookiepre}', '{authkey}', '{attachdir}'
            ), array(
                $cfg['server'],$cfg['client'],$db['server'], $db['username'], $db['password'], $db['port'], $db['name'], $db['prefix'], $cookiepre, $authkey, 'attachment'
            ), $config);
        ...
        <div class="panel panel-default">
			...
			<div class="panel-body">
				<div class="form-group">
					<label class="col-sm-2 control-label">中心服务器IP</label>
					<div class="col-sm-4">
						<input class="form-control" type="text" name="cfg[server]" value="10.2.233.21">
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label">开盘服务器名称</label>
					<div class="col-sm-4">
						<input class="form-control" type="text" name="cfg[client]" value="">
					</div>
				</div>
			</div>
		</div>
