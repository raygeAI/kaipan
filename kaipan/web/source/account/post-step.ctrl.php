...
if($step == 1) {
	...
	if(checksubmit('submit')) {
		...
		if(!empty($username) && !empty($password)) {
			...
			if (empty($basicinfo['name'])) {
				message('一键获取信息失败,请手动设置楼盘项目信息！', url('account/post-step/', array('uniacid' => $uniacid, 'step' => 3)), 'error');
			}
		...
		if(is_error($acid)) {
			message('添加楼盘项目信息失败', '', url('account/post-step/', array('uniacid' => $uniacid, 'step' => 2), 'error'));
		}
	...
	if(checksubmit('submit') && $flag == 1) {
		...
		if(empty($account)) {
			...
			if(is_error($acid)) {
				message('添加楼盘项目信息失败', '', url('account/post-step/', array('uniacid' => intval($_GPC['uniacid']), 'step' => 3), 'error'));
			}
	...
	if(!$acid) {
	message('未填写楼盘项目信息', '', url('account/post-step/', array('uniacid' => intval($_GPC['uniacid']), 'step' => 3), 'error'));
	}
