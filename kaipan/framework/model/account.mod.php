...
function uni_create_permission($uid, $type = 1) {
	...
	$uniacount = pdo_fetchall('SELECT * FROM ' . tablename('uni_account_users') . ' WHERE uid = :uid AND role = :role ', array(':uid' => $uid, ':role' => 'manager'));
	foreach($uniacount as $list) {
...
function uni_owned($uid = 0) {
	...
	if($_W['isfounder']) {
		return (array)pdo_fetchall("SELECT * FROM " . tablename('uni_account') . " ORDER BY `uniacid` DESC", array(), 'uniacid');
	} else {
		$uniacids = pdo_fetchall("SELECT uniacid FROM ".tablename('uni_account_users')." WHERE uid = '{$uid}'", array(), 'uniacid');
		if(empty($uniacids)) {
			return array();
		}
		return (array)pdo_fetchall("SELECT * FROM " . tablename('uni_account') . " WHERE uniacid IN (".implode(',', array_keys($uniacids)).") ORDER BY `uniacid` DESC", array(), 'uniacid');
	}
}


function uni_permission($uid, $uniacid) {
	global $_W;
	$uid = empty($uid) ? $_W['uid'] : $uid;
	if($_W['isfounder']) {
		return 'founder';
...
function uni_accounts($uniacid = 0) {
	...
	foreach ($types as $type) {
		$list = pdo_fetchall("SELECT a.*,b.isconnect FROM ".tablename($type['table'])." AS a LEFT JOIN " . tablename('account') . " AS b ON a.acid = b.acid WHERE a.uniacid = :uniacid", array(':uniacid' => $uniacid), 'acid');
		if (!empty($list)) {
			foreach ($list as $row) {
				$row['type'] = $type['sn'];
...
function uni_fetch($uniacid = 0) {
	...
	$account = pdo_fetch($sql, array(':uniacid' => $uniacid));
	return $account;
}
...
function uni_modules($enabledOnly = true) {
	...
	if (empty($groupid)) {
		...
		$wechatgroup = pdo_fetch("SELECT `modules` FROM ".tablename('uni_group')." WHERE id = '{$groupid}'");
		$ms = '';
...
//		$ms = "'{$ms}'";
//		$mymodules = pdo_fetchall("SELECT `module`, `enabled`, `settings` FROM ".tablename('uni_account_modules')." WHERE uniacid = '{$uniacid}' AND `module` IN ({$ms}) ORDER BY enabled DESC", array(), 'module');
//	}
//	if (!empty($mymodules)) {
...
function uni_templates() {
	...
	if (empty($groupid)) {
		...
		$wechatgroup = pdo_fetch("SELECT modules, templates FROM ".tablename('uni_group')." WHERE id = '{$groupid}'");
...
function uni_setting($uniacid = 0, $fields = '*') {
	...
	$unisetting = pdo_fetch("SELECT {$fields} FROM ".tablename('uni_settings')." WHERE uniacid = '{$uniacid}'");
	if(is_array($unisetting)) {
...
function account_weixin_interface($username, $acid = '', $token = '') {
	...
	$response = account_weixin_http($username, WEIXIN_ROOT . '/advanced/callbackprofile?t=ajax-response&lang=zh_CN', array('url' => 'http://wx.we7.cc/api.php?hash=4a5e1', 'callback_token' => '18faa31e99bad0ddf7be0760edd9d7db'));
	if (is_error($response)) {
		return $response;
	}
	$response = json_decode($response['content'], true);
