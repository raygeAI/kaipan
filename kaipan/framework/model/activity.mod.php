...
function activity_coupon_owned($uid, $filter = array(), $pindex = 1, $psize = 10) {
	...
	$rows = pdo_fetchall('SELECT COUNT(*) AS rows FROM ' . tablename('activity_coupon_record') . ' AS r JOIN '. tablename('activity_coupon') . " AS c ON r.couponid = c.couponid WHERE uid=:uid AND type=1 {$condition} GROUP BY r.couponid", array(':uid' => $uid));
	if($psize > 0) {
		$data = pdo_fetchall("SELECT COUNT(r.couponid) AS cototal,r.*,c.couponid,c.title,c.type,c.description,c.discount,c.starttime,c.endtime,c.thumb FROM ".tablename('activity_coupon_record')." AS r LEFT JOIN ".tablename('activity_coupon')." AS c ON r.couponid = c.couponid WHERE c.type = 1 AND r.uid = :uid ".$condition." GROUP BY r.couponid ORDER BY r.recid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,array(':uid' => $uid));
	} else {
		$data = pdo_fetchall("SELECT r.*,c.couponid,c.title,c.type,c.description,c.discount,c.starttime,c.endtime,c.thumb FROM ".tablename('activity_coupon_record')." AS r LEFT JOIN ".tablename('activity_coupon')." AS c ON r.couponid = c.couponid WHERE c.type = 1 AND r.uid = :uid ".$condition,array(':uid' => $uid));
	}
	return array('total' => $total,'data' => $data, 'rows' => $rows);
}


function activity_coupon_info($couponid) {
	global $_W;
	$couponid = intval($couponid);
	return  pdo_fetch("SELECT title,description,discount,starttime,endtime,`limit`,amount-dosage AS residue FROM ".tablename('activity_coupon')." WHERE `type` = 1 AND `couponid` = :couponid LIMIT 1",array(':couponid' => $couponid));
}
...
function activity_coupon_use($uid, $couponid, $module = 'system') {
	global $_W;
	$coupon = pdo_fetch("SELECT * FROM ".tablename('activity_coupon')." WHERE `type` = 1 AND `couponid` = :couponid LIMIT 1",array(':couponid' => $couponid));
	$precord = pdo_fetch("SELECT * FROM ".tablename('activity_coupon_record')." WHERE `uid` = :uid AND `couponid` = :couponid AND `status` = 1 ORDER BY granttime ASC LIMIT 1",array(':couponid' => $couponid,':uid' => $uid));
	if(empty($coupon)) {
...
function activity_token_owned($uid, $filter = array(), $pindex = 10, $psize = 0) {
	...
	$rows = pdo_fetchall('SELECT COUNT(*) AS rows FROM ' . tablename('activity_coupon_record') . ' AS r JOIN '. tablename('activity_coupon') . " AS c ON r.couponid = c.couponid WHERE uid=:uid AND type=2 {$condition} GROUP BY r.couponid", array(':uid' => $uid));
	if($psize > 0) {
		$data = pdo_fetchall("SELECT COUNT(r.couponid) AS cototal,r.*,c.couponid,c.condition,c.title,c.discount,c.type,c.starttime,c.endtime FROM ".tablename('activity_coupon_record')." AS r LEFT JOIN ".tablename('activity_coupon')." AS c ON r.couponid = c.couponid WHERE c.type = 2 AND r.uid = :uid ".$condition."   GROUP BY r.couponid ORDER BY r.recid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,array(':uid' => $uid));
	} else {
		$data = pdo_fetchall("SELECT r.*,c.couponid,c.condition,c.title,c.discount,c.type,c.starttime,c.endtime FROM ".tablename('activity_coupon_record')." AS r LEFT JOIN ".tablename('activity_coupon')." AS c ON r.couponid = c.couponid WHERE c.type = 2 AND r.uid = :uid ".$condition,array(':uid' => $uid));
	}
	return array('total' => $total,'data' => $data, 'rows' => $rows);
}


function activity_token_info($couponid) {
	global $_W;
	$couponid = intval($couponid);
	return pdo_fetch("SELECT title,description,discount,condition,starttime,endtime,`limit`,amount-dosage AS residue FROM ".tablename('activity_coupon')." WHERE `type` = 2 AND `couponid` = :couponid LIMIT 1",array(':couponid' => $couponid));
}
...
function activity_token_use($uid, $couponid, $module = 'system') {
	global $_W;
	$coupon = pdo_fetch("SELECT * FROM ".tablename('activity_coupon')." WHERE `type` = 2 AND `couponid` = :couponid LIMIT 1",array(':couponid' => $couponid));
	$precord = pdo_fetch("SELECT * FROM ".tablename('activity_coupon_record')." WHERE `uid` = :uid AND `couponid` = :couponid AND `status` = 1 ORDER BY granttime ASC LIMIT 1",array(':couponid' => $couponid,':uid' => $uid));
	if(empty($coupon)) {
...
function activity_shipping_grant($uid, $exid){
	global $_W;
	$exchange = activity_exchange_info($exid, $_W['uniacid']);
	if($exchange['num'] >= $exchange['total']) {
		return error(-1, '该实物兑换已兑换完');
	}
	$pnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('activity_exchange_trades_shipping') . ' WHERE uniacid = :uniacid AND uid = :uid AND exid = :exid', array(':uniacid' => $_W['uniacid'], ':uid' => $uid, ':exid' => $exid));
...
function activity_module_grant($uid, $exid){
	...
	if($exchange['pretotal'] > 0) {
		$activity_modules = pdo_fetch('SELECT * FROM ' . tablename('activity_modules') . ' WHERE uniacid = :uniacid AND uid = :uid AND module = :module', array(':uniacid' => $_W['uniacid'], ':uid' => $uid, 'module' => $exchange['extra']['name']));
		if($activity_modules) {
			...
			if($pnum >= $exchange['pretotal']) {
				return error(-1, '每人每' . $exchange['extra']['period'] . '天内,只能兑换' . $exchange['pretotal'] . '次');
						...
						pdo_update('activity_modules', array('available' => $activity_modules['available'] + 1), array('mid' => $activity_modules['mid']));
		} else {
			$data = array(
				'uniacid' => $_W['uniacid'],
				'uid' => intval($uid),
			...
			$activity_modules['mid'] = pdo_insertid();
				...
				$data = array('mid' => $activity_modules['mid'], 'num' => 1, 'createtime' => time());
