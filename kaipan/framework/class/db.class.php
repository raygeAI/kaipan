...
class DB {
    ...
    protected  $showDebugExcept=false;
    
    public function setShowDebugExcept($value){
        $this->showDebugExcept=$value;
    }
	...
	public function debug($output = true, $append = array()) {
		...
		if($output) {
			...
			if (!empty($append['error'][1])) {
                if($this->showDebugExcept ){
                    throw new Exception($append['error'][2],$append['error'][1]);
                }
